...
class TestStuff(unittest.TestCase):
    ...
    def test_3(self):
        ...
        t.join()
        result = []
        while not machine.io['output'].empty():
            result.append(machine.io['output'].get(block=True))

        self.assertEqual(prog,",".join([str(o) for o in result]))
    ...
    def test_4(self):
        ...
        t.start()
        t.join()
        self.assertEqual(machine.io['output'].get(block=True),1125899906842624)
    ...
    def test_5(self):
        ...
        inp = Queue()
        inp.put(77)
        machine.io = {'input': inp, 'output': Queue()}
        t = threading.Thread(target=machine.run())
        t.start()
        t.join()
        self.assertEqual(machine.io['output'].get(block=True),77)
